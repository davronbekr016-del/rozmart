...
| `static/app.js` | Каталог, карточка, корзина, оформление, «Мои заказы» | Корзина и ключ попытки лежат в `localStorage` (`rozmart.cart.v1`) и переживают перезапуск. Корзина сверяется с сервером при открытии. Максимум 99 штук одной позиции — столько же принимает сервер. Подпись Telegram уходит заголовком `X-Telegram-Init-Data` |
